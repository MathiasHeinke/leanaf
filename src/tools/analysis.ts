...
export function calcVolume(exs: ExerciseEntry[]) {
  const byExercise = exs.map((e) => ({
    ...
    topSet: e.sets.slice().sort((a, b) => b.weight * b.reps - a.weight * a.reps)[0] ?? null,
    avgRpe:
      e.sets.filter((s) => s.rpe !== undefined).reduce((a, s) => a + (s.rpe ?? 0), 0) /
      Math.max(1, e.sets.filter((s) => s.rpe !== undefined).length)
  }));
  ...
  for (const e of exs) {
    ...
    for (const m of e.muscles ?? []) byMuscle[m] = (byMuscle[m] ?? 0) + v;
  }
  ...
  const balance =
    push + pull > 0 ? Number(((pull / Math.max(1, push)) as number).toFixed(2)) : 1;
