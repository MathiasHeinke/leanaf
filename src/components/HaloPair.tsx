...
function HaloMeter({ label, value, progress, gradient, track = "rgba(0,0,0,0.08)", icon }: Halo) {
  ...
  return (
    <div className="rounded-3xl p-4 bg-white/80 dark:bg-[#0b0f14] shadow-[0_8px_24px_rgba(0,0,0,.08)] dark:shadow-[0_10px_30px_rgba(0,0,0,.45)] flex items-center gap-4">
        ...
        <g transform="translate(50,50)">
          ...
          {icon && (
            <foreignObject x={-12} y={-12} width={24} height={24}>
              <div className="w-6 h-6 flex items-center justify-center text-white/90">{icon}</div>
            </foreignObject>
          )}
        </g>
      </svg>
      <div className="flex flex-col">
        ...
        <div className="text-xs tracking-[0.25em] text-zinc-500 dark:text-zinc-400 uppercase">{label}</div>
      </div>
    </div>
  );
}
