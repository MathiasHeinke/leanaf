...
-- 5. Update any existing conversations to point to FREYA (female coaches) or ARES (male coaches)
UPDATE public.coach_conversations 
SET coach_id = CASE 
  WHEN coach_id IN ('lucy', 'vita') THEN 'freya'
  WHEN coach_id IN ('sascha', 'kai') THEN 'ares'
  ELSE coach_id
END
WHERE coach_id IN ('lucy', 'vita', 'sascha', 'kai');