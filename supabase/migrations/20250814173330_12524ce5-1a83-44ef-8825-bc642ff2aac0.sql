...
-- Step 5: Verify FREYA knowledge consolidation
INSERT INTO public.coach_analytics_summary (
  ...
  (SELECT COUNT(*)::text FROM public.coach_knowledge_base WHERE coach_id = 'freya'),
  CURRENT_DATE,
  jsonb_build_object(
    'source_coaches', ARRAY['lucy', 'vita'],
    'consolidation_date', now(),
    'expertise_areas', ARRAY['nutrition', 'supplements', 'hormones', 'female_health', 'cycle_training', 'longevity', 'lifestyle_medicine'],
    'total_entries', (SELECT COUNT(*) FROM public.coach_knowledge_base WHERE coach_id = 'freya'),
    'lucy_entries', (SELECT COUNT(*) FROM public.coach_knowledge_base WHERE coach_id = 'lucy'),
    'vita_entries', (SELECT COUNT(*) FROM public.coach_knowledge_base WHERE coach_id = 'vita'),
    'embedding_entries', (SELECT COUNT(*) FROM public.knowledge_base_embeddings ke JOIN public.coach_knowledge_base kb ON ke.knowledge_id = kb.id WHERE kb.coach_id = 'freya')
  )
