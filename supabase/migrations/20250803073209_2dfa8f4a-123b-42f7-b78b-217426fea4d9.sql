-- Critical Security Fix: Remove SECURITY DEFINER from all views (CORRECTED)
-- This migration fixes the 8 SECURITY DEFINER views identified by the linter

-- 1. Drop and recreate rolling_daily_snapshot view without SECURITY DEFINER
...
-- 2. Drop and recreate v_meal_totals view without SECURITY DEFINER (corrected column names)
...
-- 3. Drop and recreate v_summary_rolling_30 view without SECURITY DEFINER
...
-- 4. Drop and recreate v_fluids_totals view without SECURITY DEFINER
...
-- 5. Drop and recreate v_workout_totals view without SECURITY DEFINER (corrected columns)
...
-- 6. Drop and recreate v_supplement_flags view without SECURITY DEFINER
...
-- 7. Drop and recreate v_missing_summaries view without SECURITY DEFINER
...
-- 8. Drop and recreate v_user_strength_profile view without SECURITY DEFINER
