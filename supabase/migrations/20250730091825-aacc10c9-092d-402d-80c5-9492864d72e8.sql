-- Fix the assess_medical_risk function with correct parameter types
CREATE OR REPLACE FUNCTION assess_medical_risk(
  p_user_id UUID,
  p_conditions UUID[], -- Changed from TEXT[] to UUID[]
  p_custom_conditions TEXT[],
  p_medications UUID[], -- Changed from TEXT[] to UUID[]
...
BEGIN
  ...
  INSERT INTO user_medical_profile (
    ...
    v_has_conditions,
    v_takes_medications,
    p_conditions::TEXT[], -- Cast to TEXT[] for storage
    p_custom_conditions,
    p_medications::TEXT[], -- Cast to TEXT[] for storage
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    medical_conditions = p_conditions::TEXT[],
    custom_conditions = p_custom_conditions,
    medications = p_medications::TEXT[],
