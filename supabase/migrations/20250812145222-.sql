-- 1) Remove overloaded 3-arg monthly summary to fix PGRST203 ambiguity
DROP FUNCTION IF EXISTS public.compute_monthly_summary(p_user_id uuid, p_year integer, p_month integer);

-- 2) Fix weekly summary function to avoid accessing non-existent record field
...
BEGIN
  ...
  -- Aggregate workout data from daily_summaries
  SELECT 
  ...
  -- Steps and workout days from workouts table
  ...
  v_workout_days := GREATEST(COALESCE(v_workout_totals.workout_days_ds, 0), COALESCE(v_steps_totals.workout_days_w, 0));
  ...
  -- Hydration score from daily_summaries
  SELECT 
  ...
  v_compliance_metrics := jsonb_build_object(
    ...
    'workout_days', v_workout_days,
    'workout_compliance_pct', ROUND((v_workout_days::numeric / 7 * 100), 1),
  ...
  INSERT INTO public.weekly_summaries (
    ...
    v_workout_days, 7 - v_workout_days,
