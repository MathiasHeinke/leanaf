...
CREATE TABLE IF NOT EXISTS public.rag_chunk_logs (
  ...
  conversation_id TEXT NOT NULL, -- Using TEXT to match how we identify conversations
  chunk_id UUID,
  score NUMERIC,
  source_doc TEXT,
...
-- Create the dashboard view using existing conversation structure
CREATE OR REPLACE VIEW public.v_coach_dashboard AS
SELECT
  CONCAT(c.user_id::text, '-', c.coach_personality, '-', c.conversation_date::text) AS conversation_id,
  c.coach_personality AS coach,
  c.user_id,
  MIN(c.created_at) AS started_at,
  MAX(c.created_at) AS last_msg_at,
  COUNT(*) FILTER (WHERE c.message_role = 'user') AS user_msgs,
  COUNT(*) FILTER (WHERE c.message_role = 'assistant') AS coach_msgs,
...
LEFT JOIN public.tool_usage_events t ON t.conversation_id = CONCAT(c.user_id::text, '-', c.coach_personality, '-', c.conversation_date::text)
LEFT JOIN public.rag_chunk_logs r ON r.conversation_id = CONCAT(c.user_id::text, '-', c.coach_personality, '-', c.conversation_date::text)
LEFT JOIN public.admin_conversation_notes a ON a.conversation_id::text = CONCAT(c.user_id::text, '-', c.coach_personality, '-', c.conversation_date::text)
LEFT JOIN public.coach_plans cp ON cp.conversation_id = CONCAT(c.user_id::text, '-', c.coach_personality, '-', c.conversation_date::text)
GROUP BY c.user_id, c.coach_personality, c.conversation_date;
