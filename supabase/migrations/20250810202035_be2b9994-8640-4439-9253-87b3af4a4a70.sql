-- Optional idempotency for points: add client_event_id to point_activities and enforce uniqueness per user/event
ALTER TABLE public.point_activities
  ADD COLUMN IF NOT EXISTS client_event_id text;

CREATE UNIQUE INDEX IF NOT EXISTS point_activities_user_event_uidx
  ON public.point_activities (user_id, client_event_id)
  WHERE client_event_id IS NOT NULL;

-- Update function to accept optional client event id and avoid double-adding points
...
BEGIN
  ...
  -- Attempt to record point activity idempotently
  ...
  -- If duplicate event (no insert happened) and client_event_id provided, return current state without re-adding points
  IF NOT FOUND AND p_client_event_id IS NOT NULL THEN
