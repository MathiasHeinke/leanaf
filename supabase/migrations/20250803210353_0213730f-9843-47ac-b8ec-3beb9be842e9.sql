...
-- 7. Recreate v_coach_dashboard (Admin view) - using correct table names
...
SELECT 
  ...
  CASE 
    WHEN tool_usage.user_id IS NOT NULL THEN true 
    ELSE false 
  END AS used_tool,
  CASE 
    WHEN rag_usage.user_id IS NOT NULL THEN true 
    ELSE false 
  END AS used_rag,
...
) tool_usage ON cc.user_id = tool_usage.user_id
LEFT JOIN (
  SELECT DISTINCT user_id
  FROM public.rag_chunk_logs
) rag_usage ON cc.user_id = rag_usage.user_id
LEFT JOIN (
  SELECT user_id, ARRAY_AGG(DISTINCT tool_name) as tools
  FROM public.tool_usage_events
  GROUP BY user_id
) tool_list ON cc.user_id = tool_list.user_id
