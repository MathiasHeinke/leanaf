-- Enhance ensure_daily_goals to intelligently compute from profiles when targets are NULL
...
BEGIN
  -- Try to fetch today's goals first
  ...
  IF today_goals IS NULL THEN
    ...
    IF p.weight IS NOT NULL AND p.height IS NOT NULL AND p.age IS NOT NULL THEN
      ...
      END IF;
    ELSE
      v_target_cal := 2000; -- fallback if data insufficient
    END IF;
    ...
    -- Insert or upsert today's goals
