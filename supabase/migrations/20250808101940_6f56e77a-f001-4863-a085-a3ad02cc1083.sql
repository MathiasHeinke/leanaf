-- Momentum 2.0 alignment (idempotent)
...
-- Validation function and trigger for quality_score
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'ALTER TABLE public.meals ADD COLUMN ts timestamptz';
    EXECUTE 'UPDATE public.meals SET ts = created_at WHERE ts IS NULL';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE $$CREATE POLICY "Users can manage their meal images" ON public.meal_images
      FOR ALL
