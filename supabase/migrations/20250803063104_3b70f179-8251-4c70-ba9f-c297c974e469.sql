...
-- 2. Fix v_meal_totals view - corrected field names
...
-- 3. Fix v_fluids_totals view first (needed by v_summary_rolling_30)
...
-- 4. Fix v_supplement_flags view (needed by v_summary_rolling_30)
...
SELECT 
  ...
  sil.date,
  ROUND(
    (COUNT(sil.id)::numeric / NULLIF(COUNT(us.id), 0)) * 100, 0
  ) AS compliance_pct
FROM supplement_intake_log sil
LEFT JOIN user_supplements us ON us.user_id = sil.user_id AND us.is_active = true
GROUP BY sil.user_id, sil.date;

-- 5. Fix v_summary_rolling_30 view (depends on above views)
