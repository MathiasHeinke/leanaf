...
DROP FUNCTION IF EXISTS public.is_super_admin_by_email();
...
DROP FUNCTION IF EXISTS public.is_enterprise_or_super_admin(uuid);
...
DROP FUNCTION IF EXISTS public.log_security_event_enhanced(uuid, text, text, inet, text, jsonb, text);
...
DROP FUNCTION IF EXISTS public.log_failed_login_attempt(text, inet, text, text, jsonb);
...
DROP FUNCTION IF EXISTS public.check_rate_limit_progressive(text, text, integer, integer);
